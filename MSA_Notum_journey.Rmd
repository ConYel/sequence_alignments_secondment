...
hmmsearch --cut_ga --domtblout my_data/hmm_search/WIF_domtbl.out my_data/WIF.hmm my_data/oma_db > /dev/null
```
...
WIF_hmmsearch <- WIF_hmmsearch %>% 
  filter(!duplicated(target_name)) %>% 
  select(target_name, from_env_coord, to_env_coord) %>% 
  arrange(target_name)

identical(fasta_db_fil_with_len %>% names %>% str_remove(" .+"),
          WIF_hmmsearch$target_name)

#fasta_db_fil_with_len %>% head %>% Biostrings::as.list() %>% Biostrings::AAStringSet()
fasta_db_fil_with_len <- pmap(list(fasta_db_fil_with_len %>% as.list, 
          WIF_hmmsearch %>% select(target_name, from_env_coord) %>% deframe %>% as.list(),
          WIF_hmmsearch %>% select(target_name, to_env_coord) %>% deframe %>% as.list()),
     subseq) %>% Biostrings::AAStringSet()
```
...
hmmsearch_env_coord_bio2mds <- msa::msaConvert(hmmsearch_env_coord_muscle, type = "bios2mds::align")
export.fasta(hmmsearch_env_coord_bio2mds, outfile = "WIF_hmmsearch_env_coord_msa_muscle.fa", ncol = 60, open = "w")
```
check the sign_seqs_msa_muscle.fa file for the alignments
...
iqtree -s my_data/WIF_hmmsearch_env_coord_msa_muscle.fa -nt 4 -mset LG -pre my_data/my_hmmsearch_tree/WIF_hmmsearch_env_coord_tree
